...
public class BookDao {
    ...
    /**
     * 获取即将到期的图书信息，传入的条件有days，aid，aid代表当前登录用户
     *
     * @param days
     * @return
     */
    public ArrayList<History> get_HistoryListInfo3(int days, String aid) {
        ArrayList<History> tag_Array = new ArrayList<History>();
        Connection conn = DBUtil.getConnectDb();
        String sql = "select * from history where aid='" + aid + "' and status='1' and  datediff(endtime,CURRENT_DATE)<="+days;
        PreparedStatement stm = null;
        ResultSet rs = null;
        try {
            stm = conn.prepareStatement(sql);
